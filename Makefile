db: ## Subir o banco localmente.
	nerdctl compose -f ./build/db/docker-compose.yml up -d

fresh-db: ## Sobe o banco localmente mas recriando tudo do zero
	nerdctl compose -f ./build/db/docker-compose.yml down -v
	nerdctl compose -f ./build/db/docker-compose.yml up --build --remove-orphans -d
